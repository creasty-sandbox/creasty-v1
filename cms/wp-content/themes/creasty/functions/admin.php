...
==============================================================================================*/
global $pagenow;
if(!is_super_admin() || !in_array($pagenow, array('index.php', 'update-core.php')))
...
function custom_editor_settings($initArray){
	...
	$initArray['theme_advanced_blockformats'] = 'p,h3,h4,h5,h6,address,pre,code,div,dt,dd';
...
add_action('before_wp_tiny_mce', function(){
	?>
		<script>
			switchEditors.setup = function(ed){
				ed.onInit.add(function(ed){
					var editorId = ed.id;
					var textarea = document.getElementById(editorId);

					document.getElementById(editorId + '_ifr').setAttribute('tabindex', textarea.getAttribute('tabindex'));
					textarea.setAttribute('tabindex', null);
				});
				ed.onKeyPress.add(function(ed, event){
					// if its a tab key press
					if(event.keyCode == 9){
...
function add_column($column_name, $post_id){
	if('thumbnail' == $column_name){
		...
		if(isset($thum) && $thum)
			echo str_replace('<img ', '<img style="max-height: 100px; width: auto;" ', $thum);
		else
			echo __('None');
	}
}
...
function crst_add_admin_menu(){
	$main_page = add_menu_page('ヘルプ', 'ヘルプ', 'edit_users', 'help', 'menu_page_help');
