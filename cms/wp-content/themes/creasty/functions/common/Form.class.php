...
class Form_Validator {
	...
	public function __construct($form, $name) {
		...
		/*	Meta
		-----------------------------------------------*/
		$this->meta['name'] = $form->get_name($m[1]);
	...
	public function set_option($option) {
		...
		foreach ($values as $val) {
			if (!is_numeric($val) || $val < 0 || $val > $size - 1) {
	...
	/*	Rule & Filter
	-----------------------------------------------*/
	private function _filter($call, $filter, $arg = array()) {
	...
	public function filter($filter) {
		...
		$this->_filter('filter', $filter, $arg);

		return $this;
	}
	public function rule($rule, $arg = null) {
		if ($this->interrupt())
			return $this;
		...
		if (preg_match('|^[a-z_]\w+$|', $rule)) {
			if (self::$pattern[$rule]) {
				...
				if (false === $this->_filter('rule', $rule, $_arg))
					$this->reject('invalid');
			}
		} else { // regexp
