/* Prototype v1.2 | PrototypeJS v1.7 */

var Prototype = {
	ScriptFragment : '<script[^>]*>([\\S\\s]*?)<\/script>',
	JSONFilter : /^\/\*-secure-([\s\S]*)\*\/\s*$/,
	emptyFunction : function( x ){},
...
var ARRAY_CLASS = "[object Array]",
	...
	NULL_CLASS = "[object Null]",
	NUMBER_CLASS = "[object Number]",
	OBJECT_CLASS = "[object Object]",
	STRING_CLASS = "[object String]",
	UNDEFINED_CLASS = "[object Undefined]";

function isTypeObject( o ){ return typeof o == "object"; } // {}, [], null, are all TRUE

function obj2str( o ){	return Object.prototype.toString.call( o ); }

function isArray( o ){ return obj2str( o ) == ARRAY_CLASS; }
function isBlank( o ){ return /^\s*$/.test( o ); }
function isBoolean( o ){ return obj2str( o ) === BOOLEAN_CLASS; }
function isDate( o ){ return obj2str.call( o ) === DATE_CLASS; }
function isElement( o ){ return !!( o && o.nodeType === 1 ); }
function isFunction( o ){ return obj2str( o ) == FUNCTION_CLASS; }
function isNull( o ){ return o === null; }
function isNumber( o ){ return obj2str( o ) === NUMBER_CLASS; }
function isObject( o ){ return obj2str( o ) == OBJECT_CLASS; }
function isString( o ){ return obj2str( o ) === STRING_CLASS; }
function isUndefined( o ){ return o == void 0; }
function isEmpty( o ){
	if( isObject( o ) ) for( var i in o ) if( isUndefined( o[i] ) && isFunction( o[i] ) ) return true;
	return isString( o ) && ( o == "" || isNull( o ) );
}
function isset(){
	for( var i = 0, len = arguments.length; i < len; i++ ){
		var arg = arguments[i];
		if( arg == null ) // isUndefined( arg ) || isNull( arg )
			return false;
	}
	return true;
}
...
var Class = (function(){
	...
	function addMethods( source ){
		...
		if( IS_DONTENUM_BUGGY ){
			( source.toString != Object.prototype.toString ) && properties.push( "toString" );
...
*/
	function extend( destination, source ){ // do deep extend, deep clone
	...
	function Str( key, holder, stack ){
		...
		if( isTypeObject( value ) && isFunction( value.toJSON ) ) 
			value = value.toJSON( key );
		
		var _class = obj2str( value );
			...
			case 'object':
				...
				if( _class === ARRAY_CLASS ){
					for( var i = 0, length = value.length; i < length; i++ ){
						...
						partial.push( isUndefined( str ) ? 'null' : str );
					}
					...
					for( var i = 0, length = keys.length; i < length; i++ ){
						...
						isUndefined( str ) && partial.push( key.inspect( true ) + ':' + str );
					}
					partial = '{' + partial.join( ',' ) + '}';
				}
				stack.pop();
	...
	function keys( object ){
		if( !isTypeObject( object ) ) throw new TypeError();
		var results = [];
	...
	var NATIVE_JSON_STRINGIFY_SUPPORT = 
		...
		&& JSON.stringify(0) === '0' 
		&& isUndefined( JSON.stringify( Prototype.K ) );
...
Object.extend( Function.prototype, (function() {
	...
	function bind( context ){
		if( arguments.length < 2 && isUndefined( arguments[0] ) ) 
			return this;
		var $this = this, args = slice.call( arguments, 1 );
		return function(){
			var a = merge( args, arguments );
			return $this.apply( context, a );
		};
	}
	...
	function curry(){
		...
		var $this = this, args = slice.call( arguments, 0 );
		return function(){
			var a = merge( args, arguments );
	...
	function delay( timeout ){
		var $this = this, args = slice.call( arguments, 1 );
		return window.setTimeout(function(){ return $this.apply( $this, args ) }, timeout * 1000);
	}
	function defer(){
	...
	function wrap(wrapper) {
		var $this = this;
		return function(){
			var a = update( [ $this.bind( this ) ], arguments );
...
Object.extend( String, {
	interpret : function( value ){ return isNull( value ) ? '' : String( value ) },
	specialChar : {
		'\b': '\\b',
		'\t': '\\t',
		'\n': '\\n',
...
Object.extend( String.prototype, (function() {
	var NATIVE_JSON_PARSE_SUPPORT = window.JSON && isFunction( JSON.parse ) && JSON.parse( '{"test": true}' ).test;
	...
	function toQueryParams( separator ){
		...
		return match[1].split( separator || '&' ).inject( {}, function( hash, pair ){
			if( ( pair = pair.split('=') )[0] ){
				...
				if( key in hash ){
					isArray( hash[key] ) || ( hash[key] = [ hash[key] ] );
					hash[key].push( value );
				}else hash[key] = value;
			}
			return hash;
...
(function(){
	...
	function flatten(){
		return this.inject( [], function( array, value ){
			if( isArray( value ) ) return array.concat( value.flatten() );
			array.push(value);
			return array;
		});
	}
	...
	function concat(){
		var array = slice.call( this, 0 ), item, i = 0, length = arguments.length;
		for( ; i < length; i++ ){
			item = arguments[i];
			if( isArray( item ) && !( 'callee' in item ) ){
...
Object.extend( Array.prototype, {
	...
	map : function( callback ){
		var map=[];
		$A( this ).each(function( val, i ){
			if( isUndefined( val ) ) 
				map[ map.length ] = callback( val, i );
		});
		return map;
	},
	swap : function( i, j ){
	...
	shuffle : function(){
		for( var i = this.length; i; this.swap( i--, ~~( Math.random() * i ) ) );
		return this.clone();
	},
...
Object.extend( String.prototype, {
	toNumber : function( base ){
		return parseFloat( this, base ) || 0;
	},
	toPadded : function( len, pad ){
		return ( pad || "0" ).times( len - this.length ) + this;
	},
	replaceAll : function( match, str ){
		return this.split( match ).join( str );
	}
});
